...
- **Creazione listino**: ogni listino ha un **nome** (es. "Studio Rossi") e un **codice**. Se quel codice coincide con il **codice utente** di una persona (impostato quando l'admin crea/modifica il suo account), quella persona vedrà automaticamente le lavorazioni di questo listino al posto del catalogo generico in "Nuova prescrizione" — nessuna assegnazione manuale ulteriore da fare.
- **Le lavorazioni di un listino sono legate al catalogo generico**: non puoi inventare una lavorazione nuova dentro un listino — il **codice deve corrispondere esattamente** a uno già presente nel catalogo generico. Categoria e descrizione vengono ereditate automaticamente da lì; tu inserisci solo il **prezzo**. Se il codice che ti serve non esiste ancora, vai prima ad aggiungerlo in Amministrazione → Catalogo.
- **Popolare un listino manualmente**: digiti/selezioni il codice (un suggerimento a tendina mostra i codici del catalogo con relativa descrizione), inserisci il prezzo, "+ Aggiungi". Se il codice non esiste nel catalogo, o è già presente in questo stesso listino, l'app mostra un errore e non lo aggiunge.
- **Popolare un listino da Excel**: carica un file `.xlsx` con **2 colonne** — Codice, Prezzo (il prezzo è opzionale) — e la prima riga di intestazioni (verrà ignorata). Ogni riga viene validata: se il codice non esiste nel catalogo generico, o è duplicato (già nel listino, o ripetuto nel file stesso), viene scartata e conteggiata a parte nel messaggio di riepilogo — non blocca l'import delle righe valide. Fogli `.xls` e `.csv` con la stessa struttura funzionano allo stesso modo.
